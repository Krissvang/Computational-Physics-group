...
TEST_CASE("Testing the ising model")
{
  ...
  ising(n_spins, mcs, T, "r", E_avg, heatcap, M_avg,
  ...
  REQUIRE(
      heatcap == Approx(1 / (T * T) * (64 * (1 + 3 * cosh(8 / T))) / (3 + cosh(8 / T)) / (3 + cosh(8 / T)))
                     .epsilon(0.1));
...
TEST_CASE("Testing the initialize algorithm")
{
  ...
  for (int de = -8; de <= 8; de += 4)
    exp_de[de + 8] = exp(-de / T);
  initialize(n_spins, "a", T, spin_matrix, E, M, count);
  int sum_elements = spin_matrix(0, 0) + spin_matrix(1, 0) +
                     spin_matrix(0, 1) + spin_matrix(1, 1);
