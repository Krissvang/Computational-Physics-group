...
void initialize(
    int, string, double, arma::Mat<int> &, double &, double &, vector<int> &);
           ...
           vector<int> &count, int steady_start, int &count_configs)
{
  ...
  for (int de = -8; de <= 8; de += 4)
    exp_de[de + 8] = exp(-de / temperature);
...
void Metropolis(int n_spins, Mat<int> &spin_matrix,
                double &E, double &M, double *exp_de, int &count_configs)
{
  // loop over all spins
  for (int i = 0; i < n_spins; i++)
  {
